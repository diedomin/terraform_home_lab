...
resource "cloudflare_dns_record" "private_service_records" {
  for_each = { for rec in var.private_service_records : "${rec.name}_${rec.type}" => rec }
...
resource "cloudflare_dns_record" "public_service_records" {
  for_each = { for rec in var.public_service_records : "${rec.name}_${rec.type}" => rec }
